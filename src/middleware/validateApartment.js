...
const validateNewApartment = async (req, res, next) => {
  ...
  try {
    const apartment = {
      ...
      FurnitureDescription: getOptionalApartmentProp(
      ...
      EntranceDate:
        formatDateForSql(req.body.date) || formatDateForSql(Date.now()),
    ...
    if (!apartment.Town)
      return res.status(400).send("Apartment's town is missing");
    ...
    if (!apartment.TotalSqm)
      return res.status(400).send("Total sqm is missing");

    if (!req.body.type)
      return res.status(400).send("Apartment's type sqm is missing");
    if (!req.body.condition)
      return res.status(400).send("Apartment's condition is missing");
